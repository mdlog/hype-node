...
**Status hari ini (10 Mei): hari ke-10 dari 12, feature complete — semua 7 exit criteria terpenuhi + beberapa scope Wave 2 sudah dikerjakan duluan (lihat section "Beyond original scope" di bawah).**
...
|---|---|---|
| **Auth flow (SIWE + iron-session)** | ✅ Done | MetaMask connect → sign-in → cookie session. Plus auto-redirect signed-in user di `app/page.tsx` ke role home |
...
| **Fundraising tracker** | ✅ Done | **Refactored 9 Mei**: original `/fundraising/projects` endpoint deprecated upstream → migrasi ke `/currencies/{id}/fundraising` (probe top 60 currencies + flatten ke event timeline) + tabel rows mirip sosovalue.com/assets/fundraising |
...
| **Triggers field di registerIndex** | ✅ Done | UI "Rebalance triggers" di builder + `rebalanceCron` ABI field wired |
...
| **Backtest page (standalone)** | ✅ Done | Page existing (~358 baris) |
| **Settings page** | ✅ Done | Page existing (~457 baris) |
| **Chat page wiring** | ✅ Done (jauh melebihi scope) | **22 tools** terdaftar — lihat detail di section "Beyond original scope" |
| **Per-asset USD price aggregation di Portfolio** | ⚠️ Deferred ke Wave 2 | ~0.5 hari kalau dibutuhkan |
| **Logo + sponsor di tokens detail** | ✅ Done | — |
| **Empty state per tab token detail** | ✅ Done | — |
| **Research page redesign** | ✅ Done | Table layout dengan filter rail (sector/source/strength/timeframe), sort, search per investor — match design HTML |
| **WalletConnect Project ID** | ❌ Placeholder masih `YOUR_WALLETCONNECT_PROJECT_ID` | 5 menit (gratis di cloud.reown.com) |

### Beyond original scope (di-ship di Wave 1, awalnya direncanakan untuk Wave 2/3)

Selama 5–10 Mei beberapa fitur Wave 2/3 di-pull forward karena dibutuhkan untuk make chat agent menjadi powerful, end-to-end demo:

| Feature | Awalnya direncanakan | Status sekarang |
|---|---|---|
| **MCP server standalone** ([agent-service/src/mcp_server.py](../agent-service/src/mcp_server.py)) | tidak di-roadmap | ✅ 8 tools, entry point Claude Desktop / external MCP client |
| **`/api/agent/tools/health`** — real per-tool readiness probe | tidak di-roadmap | ✅ status `ok` / `degraded` / `missing_config` per tool, 16 tool dipantau, render dot warna di chat MCP panel |
| **RootData OpenAPI integration** | tidak di-roadmap | ✅ Plus tier free 1000 credits/mo, 3 chat tools (`search_rootdata`, `get_rootdata_project`, `get_rootdata_investor`). Upstream untuk fundraising data SoSoValue website |
| **Browser-sign EIP-712 SoDEX trade flow** | Wave 2 (komponen 2.2 SoDEX router integration) | ✅ Dikerjakan duluan — agent prepare → wagmi `signTypedDataAsync` di browser → server proxy `/api/sodex/submit` ke SoDEX gateway. **Server tidak pegang user private key.** Buy + sell + balance + list orders + cancel via chat |
| **Auto switchChain ke ValueChain testnet** | tidak di-roadmap | ✅ Saat user approve trade, wagmi prompt switch ke chainId 138565 dulu (otomatis add chain di MetaMask kalau belum ada) |
| **Inner-code check di /sodex/submit** | tidak di-roadmap | ✅ Catch silent reject SoDEX (outer code 0 + inner code != 0) — sebelumnya UI klaim "ORDER SUBMITTED" padahal order ditolak |
| **Single-use envelope (cegah nonce reuse)** | tidak di-roadmap | ✅ UI hapus tombol Retry, kasih instruksi resubmit prompt fresh. SoDEX nonce one-shot per signed envelope |
| **Balance enrichment (locked/available/open orders)** | tidak di-roadmap | ✅ `sodex_get_balances` tool surface available + locked breakdown + open orders summary + freshness (block time). Chat agent wajib echo verbatim — defends terhadap LLM rounding |
| **Chat fundraising tools** | Wave 2/3 | ✅ `list_funding_rounds` (cross-currency event timeline) + `get_project_fundraising` (per-project detail) + RootData search/detail untuk pre-launch projects yang tidak listed |
| **Chat agent identity** | tidak di-roadmap | ✅ Avatar agent pakai logo HypeNode (bulat 18×18) + loading splash inline saat thinking |
...
**Effort: 1 minggu** — ⚠️ **PARTIALLY ADVANCED ke Wave 1** (browser-sign + signed-envelope relay sudah jadi)

Yang **SUDAH** done (di Wave 1, untuk chat agent):
...
- [x] `submit_signed_envelope()` — accept browser signature, normalize to SoDEX wire format, forward
- [x] Inner-code rejection check + chat-friendly errors

Yang **MASIH PERLU** di Wave 2 (untuk vault):
- [ ] On-chain SoDEX adapter contract (atau pakai EOA-signed off-chain dispatch)
- [ ] Slippage protection (max 1% default per swap)
- [ ] Multi-leg execution (1 USDC deposit → 8 swap atomic)
- [ ] Failed swap rollback (partial-fill handling)
- [ ] Vault-as-signer flow — saat ini chat agent pakai user wallet sebagai signer; vault flow butuh contract-as-counterparty signing semantics
...
- ⬅️ SoDEX integration sudah verified working — ✅ done di Wave 1 (browser-sign EIP-712 + signed-envelope relay)
- ⬅️ Chat agent tool surface sudah comprehensive (22 tools incl. SoDEX trading) — ✅ done di Wave 1
...
**Status**: Wave 1 in progress, hari ke-10 dari 12 (1 Mei → 12 Mei), feature complete + scope Wave 2 (SoDEX trading via chat) di-pull forward. Tinggal polish + WalletConnect ID + per-asset USD aggregation portfolio sebelum kick-off Wave 2.
